...
#spis = ['08/12/20_13:09:49_3\n', '  23303 <ErrText>0b0 ШК mark не найден в системе</ErrText>\n', '    442 <ErrText>0b2 Некорректный владелец:FsrarID ШК mark</ErrText>\n', '     11 <ErrText>0b4 Инверсия даты: some_time для ШК mark</ErrText>\n', '    237 <ErrText>0c3 Некорректная РФУ2: FormB для ШК: mark</ErrText>\n', '     32 <ErrText>0d1 Будущая дата документа: some_date</ErrText>\n', '    264 <ErrText>0g0 По РФУ2: FormB не зарегистрирован пересорт в количестве quantity единиц</ErrText>\n', '     10 <ErrText>0h4 Инверсия даты: some_date</ErrText>\n', '    372 <ErrText>0v0 ШК mark не найден в архиве</ErrText>\n', '   2733 <ErrText>1b0 ШК mark уже зарегистрирован</ErrText>\n', '    193 <ErrText>1b2 Организация с кодом FsrarID уже имеет ШК mark</ErrText>\n', '    144 <ErrText>1b3 Для ШК doctype:mark документ не разрешен</ErrText>\n', '      5 <ErrText>1d2 Документ ещё обрабатывается</ErrText>\n', '    386 <ErrText>1h0 Документ уже зарегистрирован</ErrText>\n', '   5030 <ErrText>1v0 ШК mark выведен из оборота</ErrText>\n', '      3 <ErrText>af1 ШК mark выведен из оборота организацией FsrarID</ErrText>\n', '      8 <ErrText>hs4 Продукция c РФУ2:FormB пересорчена в течение более 90 дней</ErrText>\n', '     75 <ErrText>sql1 Взаимоблокировка</ErrText>\n', '     46 <ErrText>Зарегистрирована пересортица по штрихкодам: \n', '     47 <ErrText>Организация с кодом FsrarID зарегистрировала пересортицу по штрихкодам: \n', '     95 <ErrText>Продукция со штрихкодом mark выведена из оборота.</ErrText>\n']
##print(spis)

#parss = ps.ParseData(spis)
parss = ps.ParseData(result, time)
#parss.parser()
parss.saveintable()
